...
- [ ] A reusable `MetricStatCard` (or equivalent) renders title, large value, and accent icon with hard black border/shadow.
- [ ] A metrics strip on the board can host at least three cards (Total, Complete, Current Speed) in one horizontal composition on desktop and stacks cleanly on small screens.
- [ ] Empty / loading placeholders do not break layout when values are not yet available.
- [ ] Visual style follows the reference: white surface, solid black borders, bold typography, vibrant icon square — without copying unrelated business KPIs.
