...
Ship Taskmark’s web UI so a developer can open a local board with one command (`npx` / `npm start`) — no cloning `taskmark-frontend`, no setup wizard, and no manual path configuration when the board is already on disk.
...
- Package the Next app as a publishable npm package with a CLI bin that starts a prebuilt server against the resolved board.
...
- From a product repo with `./taskmark/` (or a dedicated board root), `npx @taskmark/ui` (or agreed package name) opens the dashboard on that board with no setup steps.
